...
final class b
{
    ...
    public function g()
    {
        if (isset($this->b))
            foreach ($this->b as $a)
                header($a);
        if (isset($this->c)) foreach ($this->c as $b)
...
}final class c
{
    ...
    private function i()
    {
        foreach ($this->d as $a => $b)
            if (!is_array($b))
                $c[] = $a . ' = ' . $b . self::a . self::a; foreach ($this->d as $a => $b) {
            if (is_array($b)) {
                $c[] = '[' . $a . ']' . self::a . self::a;
                foreach ($b as $d => $e)
                    $c[] = "\t" . $d . ' = ' . $e . self::a;
...
final class d
{
    ...
    public function b()
    {
        ...
        if (!$aa || !preg_match('~\.[a-z]{2,5}$~i', $aa)) {
            ...
            if (!file_exists($ac) && ($ad = $this->j()))
                ...
                throw new Exception($this->a->c() . $this->i() . $this->a->g(D_), 307);
        }
        if (stripos($aa, substr($this->a->c(), strripos(substr($this->a->c(), 0, -1), '/') + 1)) === 0)
            throw new Exception($this->a->c() . $this->i() . $this->a->g(D_), 307);
        if ($this->b->c(_m)) {
            if ($this->b->b('reload'))
                $this->m();
            else if ($this->b->b('logout'))
                $this->n();
                    ...
                    foreach ($aj[_G] as $ao => $ap) {
                        if ($this->q(strtolower(substr($aj[_y][$ao], strripos($aj[_y][$ao], '.') + 1)))) {
                            if (!$aj[_D][$ao]) {
                                ...
                                if (isset($ar[0]))
                                    $aq[_x] = $this->a->c() . $this->i() . substr($ah, strlen($this->a->e())) . $aq[_y];
                                else
                                    $aq[_x] = $ah . $aq[_y];
                ...
                if ($av[m_] && $av[m_] != $this->d) {
                    if (stripos($az = $this->a->g(m_), ',') && stripos($az, $av[m_]) !== false) {
                        if ($av[m_] != $this->b->c(e_))
                            $this->c->d(__ . m_, $av[m_], time() + 60 * 60 * 24 * 365, $this->a->c(), false, false, true);
                        else
                            $this->c->e(__ . m_, $this->a->c());
                    } else
                        $this->c->b(404, $this->a->n(_t, 'en'));
                }
                if ($this->a->g(D_) !== null && preg_match('~^.{1,30}$~i', $av[D_], $ar) && isset($ar[0]))
                    $this->a->h(D_, $av[D_]);
                else
                    $this->c->b(404, $this->a->n(_t, 'en'));
                unset($av[n_], $av[m_], $av[D_]);
                foreach ($av as $ao => $aA) {
                    if ($this->a->g($ao) !== null && preg_match('~^[0-9]{1,7}$~i', $aA, $ar) && isset($ar[0]))
                        $this->a->h($ao, $aA);
                    else
                        $this->c->b(404, $this->a->n(_t, 'en'));
                }
                ...
                if (isset($ar[0]) && ($aC = base64_decode(str_replace('_', 'a', $aC))) && ($ag = fopen($aD, 'w'))) {
                    ...
                    if ($aE) {
                        ...
                        if ($aq == _D)
                            $this->c->b(404, $this->a->n(_t, 'en'));
                        else if (file_exists($this->a->d() . $aq))
    ...
    private function h()
    {
        ...
        if ($a === '1')
            ...
            if ((stripos($this->b->c(b_), 'q=') === 0) && (stripos($this->b->c(c_), $this->b->c(b_)) === false))
                return 1;
            else
                return 0;
        }
                ...
                if ($g != '.' && $g != '..') {
                    if (is_file($a . $g)) {
                        if (preg_match('~^index\.(?:' . $e . ')$~i', $g)) {
                            closedir($b);
                            return $g;
                        } else if (preg_match('~\.(?:' . $e . ')$~i', $g))
                            $c[] = $g;
                        else if (preg_match('~\.(?:' . $f . ')$~i', $g))
                            $d[] = $g;
                    }
                }
            }
            ...
            while (($g = readdir($f)) !== false) {
                if ($g != '.' && $g != '..') {
                    ...
                    if (is_file($b . $g)) {
                        preg_match('~\.(?:' . $e . ')$~i', $g, $i);
                        if (isset($i[0]))
                            $h[_x] = $this->a->c() . $this->i() . substr($a, strlen($this->a->e())) . $g;
                        else
                            $h[_x] = $a . $g;
                        $h[_A] = filesize($b . $g);
                        $d[] = $h;
    ...
    private function x($a)
    {
        ...
        foreach ($a as $d)
            $b[] = $d[_y];
        asort($b); foreach ($b as $e) foreach ($a as $d)
                if ($e == $d[_y])
                    $c[] = $d;
        return $c;
    }
